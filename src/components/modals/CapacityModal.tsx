    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-sm">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-bold dark:text-gray-100">슬롯별 최대 인원 설정</h2>
          ...
          {TIME_SLOTS.filter(s => s !== '12-13').map(slot => {
            ...
            return (
              <div key={slot} className="flex items-center justify-between">
                <span className="text-sm font-medium w-20 dark:text-gray-200">{slot}시</span>
                <input
                  type="number" min={0} max={10}
                  defaultValue={setting?.max_capacity ?? 2}
                  disabled={loading === slot}
                  onBlur={e => handleChange(slot as TimeSlot, e.target.value)}
                  className="w-20 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded px-2 py-1 text-sm text-center"
                />
                <span className="text-xs text-gray-400 dark:text-gray-500">명</span>
              </div>
            )
          })}
        </div>
        <button onClick={onClose} className="mt-4 w-full border border-gray-300 dark:border-gray-600 dark:text-gray-200 rounded py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700">
          닫기
        </button>
      </div>
    </div>
