...
type Phase = 'intro' | 'connect' | 'sync' | 'full' | 'outro'
...
const CAPTION: Partial<Record<Phase, { kicker: string; headline: string; sub: string }>> = {
  intro:   { kicker: 'Realtime', headline: '새로고침\n없이 반영', sub: '한 명이 배정을 추가하면 다른 화면도 즉시 업데이트됩니다' },
  connect: { kicker: 'postgres_changes', headline: '구독 하나로\n세 테이블', sub: '배정·하이라이트·날짜예외 — 자주 바뀌는 테이블만 골라서 구독합니다' },
  sync:    { kicker: 'tenant_id 필터', headline: '내 조직\n이벤트만', sub: '필터 없이 전체 구독하면 조직 수만큼 메시지가 폭증합니다' },
  full:    { kicker: 'REPLICA IDENTITY', headline: 'DELETE도\n필터 가능', sub: 'FULL 설정 후 DELETE payload.old에 모든 컬럼이 담입니다' },
}
const BADGE: Partial<Record<Phase, string>> = {
  connect: '📡 postgres_changes',
  sync:    '🔴 LIVE',
  full:    '✅ 동기화 완료',
}
...
const calVariants = { enter: { x: '100%', opacity: 0 }, center: { x: '0%', opacity: 1 }, exit: { x: '-100%', opacity: 0 } }
const calTransition = { x: { type: 'spring' as const, stiffness: 280, damping: 30 }, opacity: { duration: 0.12 } }
...
export default function App() {
  ...
  useEffect(() => {
    if (phase === 'sync') {
      ...
      let count = 0
      const id = setInterval(() => { count++; setFillCount(count); if (count >= total) clearInterval(id) }, interval)
      return () => clearInterval(id)
    }
    if (phase === 'full')    setFillCount(ALL_ASSIGNMENTS.length)
    else if (phase !== 'sync') setFillCount(0)
  }, [phase])

  const visibleAssignments =
    phase === 'connect' ? [] :
    phase === 'sync'    ? ALL_ASSIGNMENTS.slice(0, fillCount) :
    phase === 'full'    ? ALL_ASSIGNMENTS : []

  const showLive    = phase === 'sync' || phase === 'full'
  const showCounter = phase === 'sync' || phase === 'full'
  const calKey = ['connect','sync','full'].includes(phase) ? 'cal' : phase
  ...
  return (
    <div className={`stage${IS_RECORD ? ' record' : ''}`}>
      <div className="reel">
          ...
          <div className="boardwindow">
            <div className="calslide-wrap">
              <AnimatePresence mode="popLayout">
                <motion.div key={calKey} variants={calVariants} initial="enter" animate="center" exit="exit" transition={calTransition} className="calslide">
                  <WeekGrid weekDays={WEEK_DAYS} timeSlots={SLOTS} assignments={visibleAssignments}
        ...
        <AnimatePresence>
          {showLive && (
            <motion.div className="live-badge" key="live"
          ...
          {showCounter && (
            <motion.div className="sync-counter" key="counter"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            >
              {fillCount < ALL_ASSIGNMENTS.length ? `${fillCount}개 수신 중…` : `✓ ${ALL_ASSIGNMENTS.length}개 동기화 완료`}
            </motion.div>
          )}
        </AnimatePresence>
          ...
          <AnimatePresence>
            {phase === 'outro' && (
              <motion.div className="outrowrap" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ type: 'spring', stiffness: 180, damping: 20 }}>
                ...
                <p className="tagline">새로고침 없이, 모두 함께 실시간으로</p>
