...
export function LandingShiftOn() {
  ...
  const goStart = () => navigate('/consent?vertical=food-retail')
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#0a0b10', color: '#fff', fontFamily: "-apple-system,BlinkMacSystemFont,'Pretendard','Apple SD Gothic Neo',sans-serif" }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 16 }}>
              {[
                ...
                },
                {
                  visual: (
                    <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: 10, padding: '12px 14px', marginBottom: 16 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 10 }}>
                        {([
                          { name: '김지민', color: ACCENT, slots: '화·목·토 선호' },
                          { name: '이수빈', color: '#60A5FA', slots: '월·수·금 선호' },
                          { name: '박태양', color: '#F59E0B', slots: '주말 전용' },
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 4 }}>
                        ...
                        {([
                          { init: '이', bg: 'rgba(96,165,250,0.15)', border: 'rgba(96,165,250,0.3)', color: '#60A5FA', delay: 200 },
                          { init: '김', bg: 'rgba(34,197,94,0.15)', border: 'rgba(34,197,94,0.3)', color: ACCENT, delay: 400 },
                          { init: '이', bg: 'rgba(96,165,250,0.15)', border: 'rgba(96,165,250,0.3)', color: '#60A5FA', delay: 600 },
                          { init: '김', bg: 'rgba(34,197,94,0.15)', border: 'rgba(34,197,94,0.3)', color: ACCENT, delay: 800 },
                  ...
                  title: '자동 배정',
                  desc: '역할별 필요 인원 기준을 설정하면 가능한 알바를 규칙에 맞춰 빈 슬롯에 자동으로 배정합니다.',
                },
                {
                  visual: (() => {
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '28px repeat(5, 1fr)', gap: 3, marginBottom: 10 }}>
                        ...
                        {([
                          ...
                          ...row.sel.map((selected, ci) => (
                            <div key={`${row.time}-${ci}`} style={{ height: 18, borderRadius: 3, background: selected ? 'rgba(34,197,94,0.18)' : 'rgba(255,255,255,0.04)', border: `${selected ? 2 : 1}px solid ${selected ? ACCENT : 'rgba(255,255,255,0.07)'}` }} />
                          )),
                        ]))}
                      </div>
                ...
                <Anim key={card.title} delay={i * 60}>
                  <div className="sh-card" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 18, overflow: 'hidden', height: '100%' }}>
                    {card.visual}
        ...
        {/* Pricing */}
        <section style={{ padding: '100px 24px', background: 'linear-gradient(180deg, transparent, rgba(34,197,94,0.04), transparent)' }}>
          <Anim style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <h2 style={{ fontSize: 'clamp(24px,3.5vw,34px)', fontWeight: 800, letterSpacing: '-0.5px', marginBottom: 12 }}>필요한 규모에 맞게 선택하십시오</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16, maxWidth: 860, margin: '0 auto' }}>
            {[
              { name: '무료', price: '₩0', period: '영구 무료', highlight: false, badge: null as string | null, features: ['멤버 10명', '기본 스케줄 관리', '드래그 배정', '모바일 접근'] },
              { name: 'Pro', price: '₩29,000', period: '/ 월', highlight: true, badge: '인기' as string | null, features: ['멤버 50명', '자동 알림 (SMS 100건)', '근무 시간 집계', '급여 계산', 'XLSX 내보내기'] },
              { name: 'Business', price: '₩79,000', period: '/ 월', highlight: false, badge: null as string | null, features: ['멤버 무제한', 'SMS 무제한', '다중 매장 관리', '전용 온보딩', '우선 지원'] },
            ].map((plan, i) => (
              <Anim key={plan.name} delay={i * 80}>
                <div style={{ background: plan.highlight ? 'rgba(34,197,94,0.08)' : 'rgba(255,255,255,0.03)', border: `1px solid ${plan.highlight ? 'rgba(34,197,94,0.35)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 20, padding: '28px 24px', position: 'relative', height: '100%' }}>
                  {plan.badge && (
