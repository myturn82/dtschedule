...
const MEMBER_SECTIONS: HelpSection[] = [
  {
    ...
    items: [
      { q: '월간/주간/일간 뷰를 어떻게 전환하나요?', a: '스케줄 상단의 "월·주·일" 버튼을 클릭하면 전환됩니다. 모바일에서는 타이틀 오른쪽에, PC에서는 화면 중앙 타이틀 옆에 표시됩니다.' },
      { q: '시간별/일자별 보기는 어떻게 다른가요?', a: '"시간별"은 시간대별 표로, "일자별"은 날짜마다 배정을 요약 목록으로 보여줍니다. 헤더의 토글 버튼으로 언제든 전환할 수 있고, 일자별 보기에서도 각 날짜의 "+ 등록"으로 바로 배정을 추가할 수 있습니다.' },
      { q: '다른 달·주로 이동하려면?', a: '헤더의 ‹ › 화살표로 이전/다음으로 이동합니다. 타이틀(연월 등) 텍스트를 클릭하면 연/월/일을 스크롤 휠로 골라 원하는 날짜로 바로 이동할 수 있습니다.' },
      { q: '주간 뷰에 뜨는 "2주차" 같은 표시는 뭔가요?', a: '날짜 범위 대신 그 달의 몇 번째 주인지를 보여줍니다. 한 주에 해당 월의 날짜가 4일 이상 포함되어야 그 달의 N주차로 계산됩니다.' },
      { q: '특정 사람의 배정만 보고 싶어요', a: '상단 검색창에 이름을 입력하면 해당 배정이 노란색으로 강조 표시됩니다. 월간·주간 뷰는 물론 일자별 보기에서도 동일하게 강조되며, 빈 칸으로 두면 전체를 다시 봅니다.' },
    ],
  },
  {
    id: 'm2', title: '배정 확인', icon: '👤',
    items: [
      { q: '배정 셀을 클릭하면 무엇이 보이나요?', a: '해당 날짜·시간대의 담당자, 메모, 입력항목 등 상세 정보가 팝업으로 표시됩니다.' },
...
const ADMIN_SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { q: '입력항목을 추가하려면?', a: '관리자콘솔 → "입력항목" 탭에서 텍스트·숫자·드롭다운 등 원하는 타입의 필드를 추가합니다.' },
