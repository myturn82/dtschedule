...
- 🔔 **D-1 배정알림 수동 발송 & 발송내역 조회** — 배정알림 탭의 "지금 발송" 버튼으로 관리자가 내일 배정된 멤버에게 인앱 알림을 수동 발송(자동 정시 발송·웹푸시는 현재 비활성화), 발송내역을 수신자·메시지·발송일시·읽음여부로 확인하고 이름·읽음 상태·날짜로 필터링, 잘린 메시지는 클릭해 전체 보기
