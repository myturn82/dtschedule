...
- 💬 **자체 피드백 게시판** — 헤더의 피드백 버튼으로 회원·관리자가 단순문의(예약변경 등)·오류신고·기능개선을 등록; 단순문의는 소속 조직 관리자에게, 오류신고·기능개선은 시스템 관리자에게 자동 라우팅되어 각각 "피드백" 탭(관리자콘솔/슈퍼관리자)에서 확인·답변, 답변 등록 시 작성자에게 인앱 알림 발송, 관리자는 피드백 버튼에 답변대기 건수 배지로 확인
