...
> ℹ️ **하단 설치 배너 아이콘이 DTS로 보이는 것은 정상**이다.
> Chrome이 이전 방문 시 캐싱한 아이콘을 배너에 표시한다. 실제 설치 시에는
> 현재 `manifest.webmanifest`를 읽어 LESSON:ON으로 설치된다.
> 사이트 데이터 초기화 또는 시간이 지나면 배너 아이콘도 갱신된다.

1. Android Chrome에서 배포 URL 접속
2. 주소창 우측 메뉴(⋮) → **홈 화면에 추가** 또는 **앱 설치**
3. 확인:
   - [ ] 설치 팝업 앱 이름이 **LESSON:ON**이다
   - [ ] **설치 후** 홈화면 아이콘이 **오렌지 그리드 아이콘**이다 ← 핵심 확인 항목
