...
- [x] `.env.lesson-on` 파일 작성 (`VITE_APP_ID`, `VITE_BRAND_NAME`, `VITE_SUPABASE_URL` 등)
...
- [x] `android/app/build.gradle` — `signingConfigs.release` 설정 (keystore.properties 참조)
- [x] Supabase **운영** 대시보드 → Authentication → URL 설정 → Redirect URLs → `com.dtschedule.lessonon://login-callback` 추가

### SHIFT:ON (`com.dtschedule.shifton`)
- [ ] `.env.shift-on` 파일 작성
- [ ] `package.json` — `build:shift-on`, `icon:shift-on` 스크립트 추가
- [ ] `scripts/generate-icon.js` — `VERTICAL_ICONS`에 `shift-on` 추가 (이미 있으면 확인만)
- [ ] `android/app/keystores/shift-on.keystore` 생성
- [ ] 키스토어 비밀번호 암호 관리자 백업
- [ ] Supabase 운영 대시보드 → `com.dtschedule.shifton://login-callback` 추가

### SERVE:ON (`com.dtschedule.serveon`)
- [ ] `.env.serve-on` 파일 작성
- [ ] `package.json` — `build:serve-on`, `icon:serve-on` 스크립트 추가
- [ ] `scripts/generate-icon.js` — `VERTICAL_ICONS`에 `serve-on` 추가 (이미 있으면 확인만)
- [ ] `android/app/keystores/serve-on.keystore` 생성
- [ ] 키스토어 비밀번호 암호 관리자 백업
- [ ] Supabase 운영 대시보드 → `com.dtschedule.serveon://login-callback` 추가
...
- [ ] 키스토어 비밀번호 암호 관리자 백업
- [ ] Supabase 운영 대시보드 → `com.dtschedule.classon://login-callback` 추가
...
- [ ] 키스토어 비밀번호 암호 관리자 백업
- [ ] Supabase 운영 대시보드 → `com.dtschedule.workon://login-callback` 추가
...
- [ ] 키스토어 비밀번호 암호 관리자 백업
- [ ] Supabase 운영 대시보드 → `com.dtschedule.salonon://login-callback` 추가
...
- [ ] 키스토어 비밀번호 암호 관리자 백업
- [ ] Supabase 운영 대시보드 → `com.dtschedule.careon://login-callback` 추가
...
```
# android/app/keystore.properties 내용을 해당 버티컬로 교체
...
> ⚠️ `keystore.properties`는 Git 제외 파일이므로 수동으로 교체해야 함.
...
```powershell
# cap sync가 capacitor.build.gradle의 applicationId를 VITE_APP_ID로 업데이트
